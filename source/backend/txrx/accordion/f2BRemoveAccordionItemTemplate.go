...
const (
	F2BRemoveAccordionItemTemplate = `package {{ call .Funcs.LowerCase .PackageName }}
...
func receive{{ .MessageStructName }}(ctx context.Context, ctxCancel context.CancelFunc, stores *_store_.Stores, msg interface{}) {
	...
	// The front-end removed the accordionItem so do it here.
	_message_.RemoveAccordionItemMessengerID({{ call .Funcs.DeCap .MessageStructName }}Msg.AccordionMessengerID, {{ call .Funcs.DeCap .MessageStructName }}Msg.AccordionItemMessengerID)
}
`
)
