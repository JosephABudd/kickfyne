...
type MessengerTemplateData struct {
	...
	AccordionMessageB2FAddAccordion        string
	AccordionMessageB2FAddAccordionItem    string
	AccordionMessageB2FRemoveAccordion     string
	AccordionMessageB2FRemoveAccordionItem string
	AccordionMessageF2BAddAccordion        string
	AccordionMessageF2BAddAccordionItem    string
	AccordionMessageF2BRemoveAccordion     string
	AccordionMessageF2BRemoveAccordionItem string
	Funcs                                  _utils_.Funcs
}
...
// It adds this message receiver as a listener to the Spawn{{ .PackageName }}AccordionItem message from the back-end.
func (messenger *Messenger) startReceiving() (err error) {
	...
	// Listen for the Spawn{{ .PackageName }}AccordionItem message.
	if err = _txrxchans_.AddReceiver(
		messenger,
		_message_.B2FAddAccordion{{ .PackageName }}ID,
		_message_.B2FAddAccordionItem{{ .PackageName }}ID,
		_message_.B2FRemoveAccordion{{ .PackageName }}ID,
		_message_.B2FRemoveAccordionItem{{ .PackageName }}ID,
	); err != nil {
		return
	}
...
func (messenger *Messenger) Receive(msg any) {
	switch msg := msg.(type) {
	case *_message_.B2FAddAccordion{{ .PackageName }}:
		messenger.receive{{ .AccordionMessageB2FAddAccordion }}(msg)
	case *_message_.B2FAddAccordionItem{{ .PackageName }}:
		messenger.receive{{ .AccordionMessageB2FAddAccordionItem }}(msg)
	case *_message_.B2FRemoveAccordion{{ .PackageName }}:
		messenger.receive{{ .AccordionMessageB2FRemoveAccordion }}(msg)
	case *_message_.B2FRemoveAccordionItem{{ .PackageName }}:
		messenger.receive{{ .AccordionMessageB2FRemoveAccordionItem }}(msg)
	}
}
