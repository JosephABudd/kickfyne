...
type MessengerTemplateData struct {
	...
	AppTabMessageB2FAddTabbar    string
	AppTabMessageB2FAddTab       string
	AppTabMessageB2FRemoveTabbar string
	AppTabMessageB2FRemoveTab    string
	AppTabMessageF2BAddTabbar    string
	AppTabMessageF2BAddTab       string
	AppTabMessageF2BRemoveTabbar string
	AppTabMessageF2BRemoveTab    string
	Funcs                        _utils_.Funcs
}
...
func (messenger *Messenger) Receive(msg any) {
	...
	case *_message_.B2FAddTabbar{{ .PackageName }}:
		messenger.receive{{ .AppTabMessageB2FAddTabbar }}(msg)
	case *_message_.B2FAddTab{{ .PackageName }}:
		messenger.receive{{ .AppTabMessageB2FAddTab }}(msg)
	case *_message_.B2FRemoveTabbar{{ .PackageName }}:
		messenger.receive{{ .AppTabMessageB2FRemoveTabbar }}(msg)
	case *_message_.B2FRemoveTab{{ .PackageName }}:
		messenger.receive{{ .AppTabMessageB2FRemoveTab }}(msg)
	}
}
