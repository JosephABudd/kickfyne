...
func Open{{ $panelName }}Tab(screen *_misc_.Miscellaneous, tabIcon fyne.Resource, tabLabel string) (tabMessengerID string, err error) {
	...
	tabItemContentConsumer := _types_.NewAppTabsTabItemContentConsumer(screen.Layout.Tabbar(), tabItem)
	// tabItemContentConsumer := _types_.NewAppTabsTabItemContentConsumer(screen.Layout.TabbarConsumer(), screen.Layout.Tabbar(), tabItem, spawned)
	// The {{ $panelName }} panel.
...
func Open{{ $panelName }}Tab(screen *_misc_.Miscellaneous, tabIcon fyne.Resource, tabLabel string) (startupMessenger _types_.StartupMessenger, err error) {
	...
	var appTabsTabItemContentConsumer *_types_.AppTabsTabItemContentConsumer

	appTabsTabItemContentConsumer, startupMessenger, err = api.NewAppTabsTabItemContentConsumer(
		screen.CTX,
		screen.CTXCancel,
		screen.APP,
		screen.Window,
	...
	if err == nil {
		screen.Layout.AddPanelerTabItemConsumer(nil, tabItem, appTabsTabItemContentConsumer)
	}
	return
}
{{- end }}
